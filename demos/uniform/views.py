...
from demos.uniform.forms import UniForm, HelperUniForm, LayoutUniForm, MessageResponseForm
...
def basic_test(request):
    ...
    return render_to_response('uniform/test_template.html', {
        'form': form
...
def view_helper(request):
    ...
    return render_to_response('uniform/generic_form_test.html',
        response_dictionary,
        context_instance=RequestContext(request))
...
def form_helper(request):
    if request.method == "POST":
        form = HelperUniForm(request.POST)
    else:
        form = HelperUniForm()
...
def layout_test(request):
    if request.method == "POST":
        form = LayoutUniForm(request.POST)
    else:
        form = LayoutUniForm()
